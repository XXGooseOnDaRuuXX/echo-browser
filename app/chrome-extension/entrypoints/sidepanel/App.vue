          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                Showing <strong>{{ filteredMarkers.length }}</strong> of
                {{ markers.length }} markers across {{ groupedMarkers.length }} domains
              </template>
              <template v-else>
...
function createTrigger() {
  ...
  alert('Trigger management not yet implemented');
}
...
async function run(id: string) {
  try {
    ...
    if (!result) console.warn('Playback failed');
  } catch {}
}

function edit(id: string) {
  // V3 Builder not yet implemented - show message
  alert('Workflow builder not yet implemented');
